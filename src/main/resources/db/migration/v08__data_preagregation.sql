insert into aggr_products (category_id, image_url, name, unit)
select distinct category_id, image_url, name, unit from product
...
update product p
set p.aggr_product_id = (select ap.id from aggr_products ap where ap.image_url = p.image_url and p.unit = ap.unit )
